...
const {digitalOceanStorageService} = require('./storages')
const { objectService, versionService, bucketService } = require('.');
      ...
      try {
        ...
        bb.on('file', (name, stream, info) => {
          ...
          object = {
            ...
            s3Response: digitalOceanStorageService.putObject({bucketName, ...data, stream })
          };
        });
        bb.on('close', async () => {
    ...
    try {
      ...
      bb.on('file', async(name, stream, info) => {
        ...
        objects.push({
          ...
          s3Response: digitalOceanStorageService.putObject({ ...data, bucketName, stream })
        });
      });
      bb.on('close', async () => {
   ...
   */
   async deleteObject(req, res, next) {
    try {
      ...
      const s3Response = await digitalOceanStorageService.deleteObject({...data, bucketName});
   ...
   async readObject(req, res, next) {
    try {
      ...
      if (!req.query.expiresIn && req.query.download) {
        const response = await digitalOceanStorageService.readObject({...data, bucketName});
      ...
      // Download via redirect
        ...
        const response = await digitalOceanStorageService.readSignedUrl({
   ...
   */
   async createBucket(req, res, next) {
    try {
     ...
     let response = await digitalOceanStorageService.createBucket(data)
     .then(async ()=> {
...
/**
 ...
 async headObject(req, res, next) {
  try {
    ...
    const bucketName = req.currentBucket?req.currentBucket.bucketName:undefined;
    const response = await digitalOceanStorageService.headObject({...data, bucketName});
    // Set Headers
  ...
  */
  async addMetadata(req, res, next) {
    try {
      ...
      const latest = await digitalOceanStorageService.headObject({ filePath: objPath, bucketName });
      ...
      if (!Object.keys(metadataToAppend).length) {
        ...
        const response = await digitalOceanStorageService.copyObject({...data, bucketName});
        service._setS3Headers(response, res);
        res.status(204).end();
      }
    } catch (e) {
  ...
  */
  ...
  async listObjectVersion(req, res, next) {
    try {
      ...
      const response = await digitalOceanStorageService.listObjectVersion({...data, bucketName});
      res.status(200).json(response);
    } catch (e) {
      next(errorToProblem(SERVICE, e));
    }
